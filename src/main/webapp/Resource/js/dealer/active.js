...
/************************************************* sort by online *****************************************************/
    $("#sOnline").change(function () {
        for(var n =0 ; n < oTbody.rows.length; n ++){
            if(oTbody.rows[n].cells[9].children[0].getAttribute("alt") === $("#sOnline").val() || $("#sOnline").val() == 0 ){
                oTbody.rows[n].style.display = "";
            }else{
                oTbody.rows[n].style.display = "none";
            }
...
/************************************************* sort by status *****************************************************/
    $("#sStatus").change(function () {
        for(var n =0 ; n < oTbody.rows.length; n ++){
            if(oTbody.rows[n].cells[10].children[0].getAttribute("alt") === $("#sStatus").val() || $("#sStatus").val() == 0){
                oTbody.rows[n].style.display = "";
            }else{
                oTbody.rows[n].style.display = "none";
            }
...
/*************************************************** update operation *************************************************/
    ...
    if(oTbody.rows.length > 0) {
        for (var d = 0; d < oTbody.rows.length; d++) {
            var oTdActivate = oTbody.rows[d].cells[11];
            oTdActivate.onclick = function () {
                var key = this.parentNode.cells[1].innerHTML;
                var oStatus = this.parentNode.cells[10].children[0];
                activateOrDeactivate(urlActivate, key, oStatus)
            };
            var oTdDeactivate = oTbody.rows[d].cells[12];
